...
int read_cr(int number) {
	...
	if (number == 0) {
		...
		asm volatile("mov eax, cr1":"=a"(eax));
	} else if (number == 2) {
		asm volatile("mov eax, cr2":"=a"(eax));
	} else if (number == 3) {
		asm volatile("mov eax, cr3":"=a"(eax));
	} else if (number == 4) {
...
void write_cr(int number, int value) {
	if (number == 0) {
		asm volatile("mov cr0, eax"::"a"(value));
	} else if (number == 1) {
		asm volatile("mov cr1, eax"::"a"(value));
	} else if (number == 2) {
		asm volatile("mov cr2, eax"::"a"(value));
	} else if (number == 3) {
		asm volatile("mov cr3, eax"::"a"(value));
	} else if (number == 4) {
		asm volatile("mov cr4, eax"::"a"(value));
	}
}
