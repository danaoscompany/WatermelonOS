...
void outportb(int index, char value) {
	asm volatile("out dx, al"::"d"(index),"a"(value));
}

char inportb(int index) {
	char value;
	asm volatile("in al, dx":"=a"(value):"d"(index));
	return value;
}

void outportw(int index, short value) {
	asm volatile("out dx, ax"::"d"(index),"a"(value));
}

short inportw(int index) {
	short value;
	asm volatile("in ax, dx":"=a"(value):"d"(index));
	return value;
}

void outportd(int index, int value) {
	asm volatile("out dx, eax"::"d"(index),"a"(value));
}
...
int inportd(int index) {
	int value;
	asm volatile("in eax, dx":"=a"(value):"d"(index));
	return value;
}
